...
if(altindaBlokVarMi(kedi.position.x,kedi.position.y,kedi.position.z)&&yGravity<=0){yerde=true;yGravity=0;if(ziplamayaHazir&&(keys[' ']||zipliyor)){yGravity=0.20;yerde=false;ziplamayaHazir=false;}}else{yerde=false;yGravity-=0.022;if(yGravity<-0.28)yGravity=-0.28;const yn=kedi.position.y+yGravity;if(!karakterCarpiyorMu(kedi.position.x,yn,kedi.position.z))kedi.position.y=yn;else if(yGravity<0){kedi.position.y=Math.floor(kedi.position.y)+0.25;yGravity=0;yerde=true;}else yGravity=0;}
