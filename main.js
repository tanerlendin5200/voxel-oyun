...
function setupMouse(){const c=renderer.domElement;c.addEventListener('click',()=>{if(!pointerLocked&&!isMobile)c.requestPointerLock();});document.addEventListener('pointerlockchange',()=>{pointerLocked=document.pointerLockElement===c;});document.addEventListener('mousemove',e=>{if(!pointerLocked||isMobile)return;playerRotY-=e.movementX*0.003;playerRotX-=e.movementY*0.003;playerRotX=Math.max(-1.5,Math.min(1.5,playerRotX));});c.addEventListener('mousedown',e=>{if(e.button===0&&pointerLocked)blokKirYap();if(e.button===2&&pointerLocked){blokKoyYap();e.preventDefault();}});c.addEventListener('contextmenu',e=>{e.preventDefault();});c.addEventListener('wheel',e=>{if(!pointerLocked)return;kameraMesafe+=e.deltaY*0.01;kameraMesafe=Math.max(2,Math.min(12,kameraMesafe));e.preventDefault();},{passive:false});}
...
function blokKirYap(mp){const c=mp||new THREE.Vector2(0,0);raycaster.setFromCamera(c,camera);for(const h of raycaster.intersectObjects(blockGroup.children)){if(h.object.userData?.isBlock||h.object.userData?.isDunya){const pos=h.object.userData.pos||{x:Math.round(h.point.x-h.face.normal.x*0.5),y:Math.round(h.point.y-h.face.normal.y*0.5),z:Math.round(h.point.z-h.face.normal.z*0.5)};if(!pos.x&&pos.x!==0)continue;blokSil(Math.round(pos.x),Math.round(pos.y),Math.round(pos.z));return;}}}
function blokKoyYap(){raycaster.setFromCamera(new THREE.Vector2(0,0),camera);for(const h of raycaster.intersectObjects(blockGroup.children)){if(!h.object.userData||(!h.object.userData.isBlock&&!h.object.userData.isDunya))continue;const n=h.face.normal;let bx,by,bz;if(h.object.userData.pos){bx=h.object.userData.pos.x;by=h.object.userData.pos.y;bz=h.object.userData.pos.z;}else{bx=Math.round(h.point.x-n.x*0.5);by=Math.round(h.point.y-n.y*0.5);bz=Math.round(h.point.z-n.z*0.5);}
const nx=bx+Math.round(n.x),ny=by+Math.round(n.y),nz=bz+Math.round(n.z);const px=Math.floor(kedi.position.x),py=Math.floor(kedi.position.y),pz=Math.floor(kedi.position.z);if(nx===px&&(ny===py||ny===py+1)&&nz===pz)return;if(blokVarMi(nx,ny,nz))return;blokEkle(nx,ny,nz,seciliBlok);return;}}
...
const hz2=0.043;let mx=fwd.x*hz+rgt.x*hx,mz=fwd.z*hz+rgt.z*hx;
...
if(altindaBlokVarMi(kedi.position.x,kedi.position.y,kedi.position.z)&&yGravity<=0){yerde=true;yGravity=0;if(ziplamayaHazir&&(keys[' ']||zipliyor)){yGravity=0.20;yerde=false;ziplamayaHazir=false;}}else{if(yGravity<=0){yerde=false;yGravity-=0.022;if(yGravity<-0.28)yGravity=-0.28;const yn=kedi.position.y+yGravity;if(!karakterCarpiyorMu(kedi.position.x,yn,kedi.position.z))kedi.position.y=yn;else if(yGravity<0){kedi.position.y=Math.floor(kedi.position.y)+0.25;yGravity=0;yerde=true;}else yGravity=0;}else{yerde=false;yGravity-=0.022;const yn=kedi.position.y+yGravity;if(!karakterCarpiyorMu(kedi.position.x,yn,kedi.position.z))kedi.position.y=yn;else{yGravity=0;}}}
...
const kyt=2.5+playerRotX*2,kx2=kedi.position.x-Math.sin(playerRotY)*kameraMesafe,kz2=kedi.position.z-Math.cos(playerRotY)*kameraMesafe;camera.position.set(kx2,kedi.position.y+kyt,kz2);camera.lookAt(kedi.position.x,kedi.position.y+1,kedi.position.z);
