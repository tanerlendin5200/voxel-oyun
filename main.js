...
function baslat(){
  ...
  steve=steveOlustur();steve.position.set(3,spY,3);S.add(steve);
...
function carp(x,y,z){const bx=Math.floor(x),by=Math.floor(y),bz=Math.floor(z);for(let ox=-1;ox<=1;ox++)for(let oz=-1;oz<=1;oz++)for(let oy=0;oy<=1;oy++){if(!blokV(bx+ox,by+oy,bz+oz))continue;if(x+0.2>bx+ox-0.5&&x-0.2<bx+ox+0.5&&y+0.4>by+oy-0.5&&y-0.1<by+oy+0.5&&z+0.2>bz+oz-0.5&&z-0.2<bz+oz+0.5)return true;}return false;}
...
function bkKoy(){Rc.setFromCamera(new THREE.Vector2(0,0),C);for(const h of Rc.intersectObjects(BG.children)){if(!h.object.userData?.isBlock)continue;const p=h.object.userData.pos,n=h.face.normal,nx=Math.round(p.x+n.x),ny=Math.round(p.y+n.y),nz=Math.round(p.z+n.z);if(blokV(nx,ny,nz))return;const px=Math.floor(steve.position.x),py=Math.floor(steve.position.y),pz=Math.floor(steve.position.z);for(let ox=-1;ox<=1;ox++)for(let oz=-1;oz<=1;oz++)for(let oy=0;oy<=2;oy++){if(nx===px+ox&&ny===py+oy&&nz===pz+oz)return;}blokE(nx,ny,nz,sBlok);return;}}
